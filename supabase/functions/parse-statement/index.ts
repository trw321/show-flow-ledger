...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = isPaystub
      ? `You are a pay stub parser for an AV/entertainment industry technician. Analyze the uploaded pay stub or check-stub image and extract its full structured breakdown — this is used to verify calculated pay against what was actually paid, so accuracy on the dollar figures matters more than completeness of any narrative text.

Extract:
- employer: the paying company/payroll agency name as printed
- payPeriodStart / payPeriodEnd: the pay period date range (YYYY-MM-DD each), not the check/deposit date
- totalHours: total hours paid this stub, if a single total is shown
- hoursBreakdown: if the stub itemizes hours by date and type (ST/OT/DT — straight/overtime/double time), one entry per date+type with date (YYYY-MM-DD), hours, and type. Omit if the stub doesn't break hours down this way.
- hourlyRate: the base hourly rate, if shown
- grossPay: gross earnings before any deductions
- duesAmount: union/local dues deducted, if a labeled line item shows it
- taxAmount: total tax withheld (combine federal/state/local/FICA if separately listed into one number), if shown
- vacationAmount: vacation/fringe pay, if separately listed as an addition
- otherDeductions: anything deducted that isn't dues or tax (e.g. benefits, garnishment), summed into one number, if any
- netPay: the actual take-home/deposited amount

Use null for any field not legible or not present on this stub — do not guess or estimate a value that isn't actually shown.`
      : isTimesheet
...
ONE PAY STUB IS ONE TRANSACTION. A pay stub, earnings statement or payroll advice represents a single paycheck, even when it lists several earnings rows (e.g. "Regular Pay", "Overtime", "Double Time", "Vacation Pay", "Holiday", "L16 Vac Pay"). Those rows are components of that one paycheck — never emit a separate transaction per earnings row. Emit exactly ONE transaction, and set its amount to the NET PAY (the take-home figure, usually labelled "Net Pay", "Net Check" or "Direct Deposit") — never the gross, never a subtotal, never an individual earnings row. Return multiple transactions only when the document genuinely shows separate payments, such as several distinct deposits on a bank statement.

PAYMENT NOTE DATE/HOURS BREAKDOWN — payroll payment notes and stubs often itemize the hours covered by that one payment per date, e.g. "8/16 8 ST 1 OT, 8/17 8 ST, 8/18 8 ST" (ST = straight/standard time, OT = overtime, DT = double time). When you see this pattern, extract a "breakdown" array on that transaction: one entry per date+hour-type pair found, with date (YYYY-MM-DD, inferring the year from the payment date if not given), hours (the number), and type ("ST", "OT", or "DT"). This is the single most useful signal for matching a payment to the exact shifts it covers, so extract it whenever present — do not skip it just because the transaction already has other fields filled in.

NEVER INVENT THE BREAKDOWN. Only include an entry when a specific calendar date is printed next to specific hours on the document. If the document shows only period totals (e.g. "Regular Pay 24.00 hours" for a week, with no per-day rows), you must omit the breakdown field entirely — do NOT split, distribute, spread or infer a total across dates, and do not assume standard 8-hour days. A fabricated date/hours split is far worse than no breakdown at all, because it silently mis-matches a payment to the wrong shifts. When in doubt, omit.
...
If you can't determine the date, use today's date. Return ONLY valid JSON.`
    ...
    const userPrompt = isPaystub
      ? "Extract the full structured breakdown from this pay stub image: employer, pay period, hours, rate, gross, dues, tax, vacation, and net pay."
      : isTimesheet
      ...
      : isIncome
      ? "Extract all income/payment/deposit transactions from this bank statement, invoice, or pay stub/payment notification image. If this is a pay stub or earnings statement, return exactly ONE transaction whose amount is the net (take-home) pay — not one per earnings row and not the gross. Return a JSON array of objects with fields: client, description, amount, date, invoiceNumber, and breakdown (per-date ST/OT/DT hours) if the payment note itemizes which dates/hours it covers."
    ...
    const toolDef: GatewayToolDef = isPaystub
      ? {
          name: "extract_paystub",
          description: "Extract the structured pay breakdown from a pay stub image",
          parameters: {
            type: "object",
            properties: {
              employer: { type: ["string", "null"] },
              payPeriodStart: { type: ["string", "null"], description: "YYYY-MM-DD" },
              payPeriodEnd: { type: ["string", "null"], description: "YYYY-MM-DD" },
              totalHours: { type: ["number", "null"] },
              hoursBreakdown: {
                type: ["array", "null"],
                description: "Per-date/type hours if the stub itemizes them (ST/OT/DT). Null if only a single total is shown.",
                items: {
                  type: "object",
                  properties: {
              ...
              duesAmount: { type: ["number", "null"] },
              taxAmount: { type: ["number", "null"] },
              vacationAmount: { type: ["number", "null"] },
              otherDeductions: { type: ["number", "null"] },
              netPay: { type: ["number", "null"] },
            },
            required: ["employer", "payPeriodStart", "payPeriodEnd", "totalHours", "hoursBreakdown", "hourlyRate", "grossPay", "duesAmount", "taxAmount", "vacationAmount", "otherDeductions", "netPay"],
            additionalProperties: false,
          },
        }
      : isTimesheet
